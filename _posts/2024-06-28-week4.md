...
Up until this point, the only way I was evaluating how well my networks were doing was through loss and visually comparing the output to my labels, so I decided to try to evaluate some of the outputs in a more tourough way. I first started with the original architecture as a benchmark.


4) Using the Network(s) on the Small Swarm:
